...
% 4) ideal residual vs. extracted residual (several).
...
function [x_1A,x_1B,x_2,x_2A,x_2S,x_3L,x_3Q,x_Ssa,x_Ssm] = test2_ideal(t,x,span,trend_deg,lwlr_annual,T_annual,h,cat_ind_cell,M1_A,M1_B,M2,M2_A,M2_S,M3,M1_A_cv,M1_B_cv,M2_cv,M2_A_cv,M2_S_cv,M3_cv,file_name,ideal_solve_EN)
%test2_ideal - Description
%
% Syntax: [x_1A,x_1B,x_2,x_2A,x_2S,x_3L,x_3Q,x_Ssa,x_Ssm] = test2_ideal(t,x,span,trend_deg,lwlr_annual,T_annual,h,cat_ind_cell,M1_A,M1_B,M2,M2_A,M2_S,M3,M1_A_cv,M1_B_cv,M2_cv,M2_A_cv,M2_S_cv,M3_cv,file_name,ideal_solve_EN)
%
% Long description
arguments
...
% Error (MSE) of extracted residual to ideal residual
...
for m_name = METHOD_NAME
    output.(m_name).res2res_RMSE = std(output.(m_name).residual - x.residual,0,"omitnan");
    output.(m_name).cm2cm_RMSE = std(output.(m_name).trend + output.(m_name).season - x.trend - x.season,0,"omitnan");
    output.(m_name).res2res_CC = corr(output.(m_name).residual, x.residual,'type','Pearson','rows','pairwise');
...
%%% 4. The fraction of variance explained (EV) by each component and the
