...
[eof_maps,pc,expv] = eof(sst_var(TF_lon_range,TF_lat_range,TF_time_range),n_eof);
...
%%% Create figure.
for num_EOF = 1:3
    EOF_fig(num_EOF,"Global",lon(TF_lon_range),lat(TF_lat_range),time_month(TF_time_range),eof_maps,pc,expv,1)
end

%% the Pacific Ocean (85°33'S ~ 65°44'N, 99°10'E -> 180°+78°08'E)
...
%%% Create figure.
for num_EOF = 1:3
    EOF_fig(num_EOF,"Pacific",lon(TF_lon_range),lat(TF_lat_range),time_month(TF_time_range),eof_maps,pc,expv,1)
end

%% the Atlantic Ocean (60°S ~ 65°N, 180°+78°08'E -> 20°E)
...
%
TF_lon_range = (lon_0 > -80 & lon_0 < 40);
TF_lat_range = lat > -67 & lat < 67;
TF_time_range = datetime(1900,1,1) < time_month & time_month < datetime(2020,12,30);
...
n_eof = 3; % only calculates the first n modes of variability
[eof_maps,pc,expv] = eof(sst_var_0(TF_lon_range,TF_lat_range,TF_time_range),n_eof);
% Optional scaling of Principal Components and EOF maps
for k = 1:size(pc,1)
   % Find the the maximum value in the time series of each principal component:
   maxval = max(abs(pc(k,:)));
...
%%% Create figure.
for num_EOF = 1:3
    EOF_fig(num_EOF,"Atlantic",lon_0(TF_lon_range),lat(TF_lat_range),time_month(TF_time_range),eof_maps,pc,expv,1)
end
...
function [] = EOF_fig(num_EOF,title_str,lon,lat,time_month,eof_maps,pc,expv,TF_export)
    arguments
        num_EOF
        title_str
        lon
    ...
    title(t_axes,sprintf("\\bf principal components (EOF-%d), expv = %.1f \\%%",num_EOF,expv(num_EOF)),"Interpreter","latex");
